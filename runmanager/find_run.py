...
def init():
    ...
    if ((argc != 2) and (argc != 8)):
        print(
            "Usage: " + sys.argv[0] + " <site> [erf id] [sgt var id] [rup var id] [vel id] [frequency] [source frequency]")
    ...
    for s in match_states:
        ...
        if (matches != None):
            for match in matches:
                # print "Checking ID %d" % match.getRunID()
                sgt_time = int(time.mktime(time.strptime(
                    match.getSGTTime(), '%Y-%m-%d %H:%M:%S')))
                if (pref_match == None):
                    pref_match = match
                else:
                    pref_time = int(time.mktime(time.strptime(
                        pref_match.getSGTTime(), '%Y-%m-%d %H:%M:%S')))
    ...
    if (need_clone == True):
        ...
        if (clone == None):
            ...
            if (cloneLFNs(pref_match, clone, info.awp) != 0):
                rm.rollbackTransaction()
                print("Failed cloning LFNs for run %d." %
                      (pref_match.getRunID()))
                return 1
            else:
